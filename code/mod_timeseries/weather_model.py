...
class ProcessData:
    ...
    def process_minmax(self):
        ...
        output_file_name = ""
        if self.data_type == 'max':
            output_file_name="predict_file_json\\max\\"+str(predict_month.values[0])+"-"+str(predict_day.values[0])+" "+self.data_type
        elif self.data_type == 'min':
            output_file_name="predict_file_json\\min\\"+str(predict_month.values[0])+"-"+str(predict_day.values[0])+" "+self.data_type
